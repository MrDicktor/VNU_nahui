...
from tests.parser.fixtures import (
    fake_html_fixture,
    expected_data_fixture,
    fake_html_error_fixture,
    fake_html_five_days,
    expected_data_five_days_fixture,
)
...
class TestParser:
    ...
    async def test_parser(self):
        ...
        result = await parser.parse(
            "13.04.2026", "Понеділок", 1, "08:30", "09:50", self.row
        )
    ...
    async def test_parser_no_teacher(self):
        parser = Parser()
        result = await parser.parse(
            "13.04.2026", "Вівторок", 2, "10:10", "11:30", self.no_teacher_row
        )
...
@pytest.mark.asyncio
async def test_get_lessons_data_five_days(
    mocker, fake_html_five_days, expected_data_five_days_fixture
):
